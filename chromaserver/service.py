        ...
        for role_key, agent_name in agents:
            ...
            try:
                ...
                append_chromaserver_event(
                    ...
                    level="ERROR",
                    payload={"role_key": role_key, "agent_name": agent_name, "workspace_id": base_workspace_id, "error": f"{type(e).__name__}: {e}", "cost_ms": _now_ms() - t0},
                )
                raise
        out = {"ok": True, "created": created, "cost_ms": _now_ms() - started_ms, "ts": _timestamp()}
