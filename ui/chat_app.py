...
def _清理蚁后聊天记录(*, base_dir: str) -> int:
...
def main() -> None:
    ...
    """
    资源根目录 = os.path.dirname(__file__)
    项目根目录 = os.path.dirname(资源根目录)
    load_dotenv(os.path.join(项目根目录, ".env"))
    _清理蚁后聊天记录(base_dir=项目根目录)
    # 数据调度中心：统一接收后台动态消息 + 提供 UI 事件流与快照
    data_center = DataCenter(base_dir=项目根目录)
    data_center.start()
    ...
    avatar_dir = os.path.join(资源根目录, "resources", "animations")
    ...
    def on_refresh_db() -> None:
        ...
        if not ok:
            dialog.更新系统消息(text="后台运行时仍在初始化，暂无法刷新数据库连接。")
            return
        dialog.更新系统消息(text="已发起向量数据库连接刷新（后台执行）。")

    dialog.set_on_refresh_db(on_refresh_db)

    def _取头像(agent_name: str) -> object:
        key = resolve_avatar_key(agent_name)
        file_name = avatar_files.get(key, avatar_files["worker"])
        return avatars.get(file_name)

    def _渲染事件(ev: DataEvent) -> None:
        t = ev.type
        p = ev.payload or {}
        ...
        if t == "system_message":
            ...
            if msg_text:
                dialog.更新系统消息(text=msg_text)
            return

        if t == "agent_status":
            agent_name = str(p.get("agent_name") or "")
            status = str(p.get("status") or "")
            dialog.ensure_agent_item(name=agent_name, avatar=_取头像(agent_name))
        ...
        if t == "user_reply_stream":
            ...
            if delta:
                dialog.add_user_reply_stream(name=agent_name, delta=delta, avatar=_取头像(agent_name))
            return
        ...
        if t == "user_reply":
            ...
            if reply_text:
                dialog.add_user_reply(name=agent_name, text=reply_text, avatar=_取头像(agent_name))
            return
        ...
        if t == "group_message":
            ...
            if msg_text.strip():
                if agent_name == "系统":
                    ...
                    dialog.add_group_message(name=agent_name, text=msg_text, avatar=_取头像(agent_name))
            return
        ...
        if t == "agent_message":
            ...
            msg_text = str(p.get("text") or "")
            if msg_text.strip():
                dialog.add_agent_message(name=agent_name, text=msg_text, avatar=_取头像(agent_name))
            return
    ...
    def pump_events() -> None:
        ...
        try:
            ...
            # 兜底：渲染错误不应导致 UI 主循环退出
            dialog.status_var.set("异常")
            dialog.更新系统消息(text=f"UI渲染异常：{e}")
        # 使用 Tk 定时器实现“刷新节拍”；80ms 更适合流式增量展示
        dialog.root.after(80, pump_events)

    def on_close() -> None:
        nonlocal 关闭打点起点
        关闭打点起点 = time.perf_counter()
        _打印停止打点(t0=关闭打点起点, 阶段="UI关闭开始")
        dialog.status_var.set("停止中")
        _打印停止打点(t0=关闭打点起点, 阶段="UI销毁窗口 root.destroy 开始")
        dialog.root.destroy()
        _打印停止打点(t0=关闭打点起点, 阶段="UI销毁窗口 root.destroy 结束")
    ...
    try:
        ...
        t0 = 关闭打点起点 or time.perf_counter()
        _打印停止打点(t0=t0, 阶段="UI退出主循环 mainloop 返回")
        _打印停止打点(t0=t0, 阶段="UI调用 bridge.stop 开始")
        bridge.stop()
        _打印停止打点(t0=t0, 阶段="UI调用 bridge.stop 结束")
