...
class ChromaAdminPanel(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        self.tree.column("workspace_id", width=220, anchor="w")
    ...
    def _on_init_db(self) -> None:
        ...
        def _ok(res: dict[str, Any]) -> None:
            ...
            if isinstance(created, list):
                self._append_line(f"created={len(created)}")
            if isinstance(failed, list) and failed:
                self._append_line(f"failed={len(failed)}")
                for it in failed:
                    if isinstance(it, dict):
                        self._append_line(f"- {it}")
            self._on_init_db_ok()

        def _err(e: Exception) -> None:
            self._on_init_db_error(e)

        self._run_async(c.init_db(preload_system_prompts=preload), on_ok=_ok, on_err=_err)
