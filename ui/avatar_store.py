...
def resolve_avatar_key(agent_name: str) -> str:
    ...
    if n in {"king"} or n.startswith("King_") or n.startswith("蚁王"):
        return "king"
    if n in {"queen"} or n.startswith("Queen_") or n.startswith("蚁后"):
        return "queen"
    if n in {"soldier"} or n.startswith("Soldier_") or n.startswith("兵蚁"):
        return "soldier"
    if n in {"user", "用户", "你"}:
        return "user"
    if n.startswith("Worker_") or n.startswith("工蚁"):
        return "worker"
    return "worker"
