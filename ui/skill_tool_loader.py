...
class SkillToolLoaderApp:
    ...
    def _get_assigned_skill_set(self) -> set[str]:
        ...
        if not rk:
            return set()
        folder = os.path.join(self.base_dir, "agents", "skills", rk)
    ...
    def _on_add_skill_to_agent(self) -> None:
        ...
        if s is None:
            ...
            return
        dest_dir = os.path.join(self.base_dir, "agents", "skills", rk)
        os.makedirs(dest_dir, exist_ok=True)
        dest_script = os.path.join(dest_dir, f"{skill_key}.py")
        dest_doc = os.path.join(dest_dir, f"{skill_key}.md")
        if os.path.exists(dest_script):
            messagebox.showinfo("提示", f"该 Agent 已有技能：{skill_key}")
            return
        try:
            shutil.copyfile(s.script_path, dest_script)
            shutil.copyfile(s.doc_path, dest_doc)
    ...
    def _on_remove_skill_from_agent(self) -> None:
        ...
        skill_key = str(sel[0])
        folder = os.path.join(self.base_dir, "agents", "skills", rk)
        script_path = os.path.join(folder, f"{skill_key}.py")
        doc_path = os.path.join(folder, f"{skill_key}.md")
        try:
