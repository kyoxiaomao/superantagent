...
def load_roles(base_dir: str | None = None) -> dict[str, RoleInfo]:
    ...
    for role_key, role_cfg in (agent_cfg or {}).items():
        ...
        if not isinstance(role_cfg, dict):
            raise ValueError(f"{role_key} 配置格式不正确，应为字典结构")
        rcfg = role_cfg
        name_raw = rcfg.get("name")
        if not str(name_raw or "").strip():
            raise ValueError(f"{role_key} 的 name 不能为空")
        if "max_iters" not in rcfg:
            raise ValueError(f"{role_key} 缺少 max_iters 配置")
        name = str(name_raw).strip()
        max_iters = int(rcfg.get("max_iters"))
        heartbeat = rcfg.get("heartbeat")
        hb = heartbeat if isinstance(heartbeat, dict) else {}
        sys_prompt_raw = prompt_cfg.get(role_key)
        if not str(sys_prompt_raw or "").strip():
            raise ValueError(f"{role_key} 的 system prompt 不能为空")
        sys_prompt = str(sys_prompt_raw)
