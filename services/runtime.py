...
- 用户输入只直达蚁后（queen），蚁后直接回复用户；
- 若需要把信息发送到群聊，由蚁王（king）在自己的 update() 中决定并执行发言。
"""
...
def iter_role_agents(colony: Any) -> list[tuple[str, Any]]:
    ...
    return [
        ("king", getattr(colony, "king", None)),
        ("queen", getattr(colony, "queen", None)),
        ("soldier", getattr(colony, "soldier", None)),
        ("emotion_worker", getattr(colony, "emotion_worker", None)),
        ("browser_worker", getattr(colony, "browser_worker", None)),
        ("doc_worker", getattr(colony, "doc_worker", None)),
    ]
...
class ColonyRuntime:
    ...
    async def submit_user_text(self, text: str) -> str:
        ...
        queen = getattr(self.colony, "queen", None)
        if queen is None:
            return ""
        ...
        try:
            ...
            if reply_text:
                if not self._enable_ui_events:
                    self._message_center.append_user_queen(role="assistant", name=str(getattr(queen, "name", "蚁后")), content=reply_text)
            if self._enable_ui_events and reply_text:
                await self.ui_event("user_reply", text=reply_text, agent_name=str(getattr(queen, "name", "蚁后")))
            return reply_text
        except Exception as e:
            err = f"蚁后回复失败：{e}"
            log_event(event_type="queen_reply_error", agent=str(getattr(queen, "name", "蚁后")), payload={"error": err}, level="ERROR")
            if self._enable_ui_events:
    ...
    async def ui_event(self, ui_event: str, **metadata: Any) -> None:
        if not self._enable_ui_events:
            return
        reporter = getattr(self.colony, "queen", None) or getattr(self.colony, "king", None) or (self.participants[0] if self.participants else None)
