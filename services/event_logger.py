...
def _min_level_rank() -> int:
    return _level_rank(os.getenv("EVENT_LOG_MIN_LEVEL", "INFO"))


def _确保事件日志路径() -> tuple[str | None, str]:
    with _lock:
    ...
    run_name = os.getenv("EVENT_LOG_RUN_NAME", "").strip() or "auto"
    init_event_logger(base_dir=base_dir, run_name=run_name)
    with _lock:
        return _state.get("path"), _state.get("run") or ""
...
def log_event(
    *,
    ...
    path, run_name = _确保事件日志路径()
    if not path:
        return
    if _level_rank(level) < _min_level_rank():
        return
