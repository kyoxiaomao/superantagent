...
class AntColony:
    ...
    def participants(self) -> list[Any]:
        return [self.king, self.soldier, self.emotion_worker, self.browser_worker, self.doc_worker]
...
def create_colony(model_bundle: ModelBundle) -> AntColony:
    return AntColony(
        king=create_king_agent(model_bundle),
        soldier=create_soldier_agent(model_bundle),
        emotion_worker=create_emotion_worker_agent(model_bundle),
        browser_worker=create_browser_worker_agent(model_bundle),
        doc_worker=create_doc_worker_agent(model_bundle),
    )
