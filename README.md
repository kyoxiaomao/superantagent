...
本项目基于 AgentScope + Tkinter + Pillow 构建一个“桌宠式多智能体群聊系统”。系统内置蚁王_特鲁/蚁后_瑟拉/兵蚁_阿瑞/工蚁_莱特/工蚁_诺瓦/工蚁_里德等角色：用户通过 UI 或终端输入消息，蚁后_瑟拉负责直接回复；蚁王_特鲁在后台周期性 `update()` 做群聊话题与任务调度；长期记忆通过独立进程 `chromaserver` 托管（Chroma 持久化 + JSONL 回退 + ReMe/FlowLLM 记忆栈）。
...
启动后在终端输入文本，蚁后_瑟拉会直接回复；输入 `exit` 退出。
