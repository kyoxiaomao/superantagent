...
def create_browser_worker_agent(model_bundle: ModelBundle) -> object:
    toolkit = Toolkit()
    load_skills(toolkit, role_key="browser_worker")

    def reload_skills() -> list[str]:
        return load_skills(toolkit, role_key="browser_worker")

    def list_skills() -> dict[str, list[str]]:
        return list_skill_artifacts(role_key="browser_worker")
    ...
    async def update(ctx: UpdateContext) -> None:
        ...
        浏览器工蚁未来常见扩展：监控浏览器/搜索队列、群聊内检索请求识别、工具可用性巡检等；
