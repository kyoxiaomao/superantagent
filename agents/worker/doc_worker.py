...
def create_doc_worker_agent(model_bundle: ModelBundle) -> object:
    toolkit = Toolkit()
    load_skills(toolkit, role_key="doc_worker")

    def reload_skills() -> list[str]:
        return load_skills(toolkit, role_key="doc_worker")

    def list_skills() -> dict[str, list[str]]:
        return list_skill_artifacts(role_key="doc_worker")
    ...
    async def update(ctx: UpdateContext) -> None:
        ...
        文档工蚁未来常见扩展：文档生成队列、任务进度汇总、群聊需求识别与结构化等；
